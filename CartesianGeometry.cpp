...
CartesianPoint CartesianGeometry::matrixTimesCartesianPoint(const CartesianPoint & _point, const Matrix & _rotationMatrix) {
	double x = _rotationMatrix.getij(0, 0)*_point.getX() + _rotationMatrix.getij(0, 1)*_point.getY() + _rotationMatrix.getij(0, 2)*_point.getZ();
	double y = _rotationMatrix.getij(1, 0)*_point.getX() + _rotationMatrix.getij(1, 1)*_point.getY() + _rotationMatrix.getij(1, 2)*_point.getZ();
	double z = _rotationMatrix.getij(2, 0)*_point.getX() + _rotationMatrix.getij(2, 1)*_point.getY() + _rotationMatrix.getij(2, 2)*_point.getZ();
...
CartesianPoint CartesianGeometry::matrixTransposeTimesCartesianPoint(const CartesianPoint & _point, const Matrix & _rotationMatrix) {
	double x = _rotationMatrix.getij(0, 0)*_point.getX() + _rotationMatrix.getij(1, 0)*_point.getY() + _rotationMatrix.getij(2, 0)*_point.getZ();
	double y = _rotationMatrix.getij(0, 1)*_point.getX() + _rotationMatrix.getij(1, 1)*_point.getY() + _rotationMatrix.getij(2, 1)*_point.getZ();
	double z = _rotationMatrix.getij(0, 2)*_point.getX() + _rotationMatrix.getij(1, 2)*_point.getY() + _rotationMatrix.getij(2, 2)*_point.getZ();
	return CartesianPoint(x, y, z);
}
...
Matrix CartesianGeometry::getRotationMatrix(double degrees, const CartesianPoint & _axis) {
	...
	// rotation matrix
	Matrix m(3, 3);
	m.setij(0, 0, tRad * n[0] * n[0] + cosRad);
	m.setij(0, 1, tRad * n[0] * n[1] - sinRad * n[2]);
	m.setij(0, 2, tRad * n[0] * n[2] + sinRad * n[1]);
	m.setij(1, 0, tRad * n[0] * n[1] + sinRad * n[2]);
	m.setij(1, 1, tRad * n[1] * n[1] + cosRad);
	m.setij(1, 2, tRad * n[1] * n[2] - sinRad * n[0]);
	m.setij(2, 0, tRad * n[0] * n[2] - sinRad * n[1]);
	m.setij(2, 1, tRad * n[1] * n[2] + sinRad * n[0]);
	m.setij(2, 2, tRad * n[2] * n[2] + cosRad);
